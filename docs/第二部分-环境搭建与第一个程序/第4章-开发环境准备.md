...
接下来安装本书前几章需要的 Python 包：

```bash
python -m pip install langgraph langchain langchain-ollama langchain-deepseek python-dotenv
```
...
然后拉取一个本书前期示例可以使用的模型。这里用 `qwen2.5:7b-instruct` 作为示例：

```bash
ollama pull qwen2.5:7b-instruct
```

如果你的机器内存较小，可以选择更小的模型；如果这个标签在你的环境中不可用，也可以换成 Ollama 模型库里已有的其他对话模型。后续代码里的 `model="qwen2.5:7b-instruct"` 要和你实际拉取的模型名一致。
...
接下来再用 Python 测试 LangChain 对 Ollama 的封装。新建 `chapter04_check_ollama.py`：
...
```bash
python chapter04_check_ollama.py
```
...
然后新建 `chapter04_check_deepseek.py`：
...
```bash
python chapter04_check_deepseek.py
```
